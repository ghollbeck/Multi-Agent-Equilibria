...
def parse_json_with_default(response_str: str, default: dict, context: str) -> dict:
    ...
    try:
        ...
        content_match = re.search(r'"processed_information"\s*:\s*"([^"]*)"', response_str)
        if content_match:
            salvaged = default.copy()
            salvaged['processed_information'] = content_match.group(1)
            print(f"❌ [parse_json_with_default] Salvaged processed_information in {context}.")
            return salvaged
        return default
